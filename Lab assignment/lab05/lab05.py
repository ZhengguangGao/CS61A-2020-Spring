...
def closer_city(lat, lon, city1, city2):
    ...
    distance1 = distance(dest,city1)

    distance2 = distance(dest,city2)
...
def sprout_leaves(t, values):
    ...
    if is_leaf(t):
    	...
    	return tree(label(t),[tree(e) for e in values])

    new_branch = [sprout_leaves(branch,values) for branch in branches(t)]
...
def add_chars(w1, w2):
    ...
    if len(w1) == 1: 
    	...
    	if w1[0] == w2[0]:
    		...
    		return w2[0] + add_chars(w1,w2[1:])
    #it may exists a better solution, who interested in it can reference the answer provided by UBC
...
def add_trees(t1, t2):
    ...
    if len(branch1) < len(branch2):
    	...
    	branch1 += [None for _ in range(len(branch1),len(branch2))]

    if len(branch1) > len(branch2):

    	branch2 += [None for _ in range(len(branch2),len(branch1))]
