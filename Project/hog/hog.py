...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    count = 3 ** (player_score + opponent_score)

    last_count = count % 10

    while count > 0:
        first_count = count % 10
        count = count // 10
    ...
    def helper(score0,score1):
        if who == 0:
            if score0-prev_score > prev_high:
                current_high = score0-prev_score
        ...
        if who == 1:
            if score1-prev_score > prev_high:
                current_high = score1-prev_score
                print(current_high, "point(s)! That's the biggest gain yet for Player", who)
            else:
                current_high = prev_high
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    max_score = 0
    while i <= 10:
        average_dice = make_averaged(roll_dice,num_samples)(i,dice)

        if average_dice > max_score:
            max_score = average_dice
            opt_number = i
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    rolls_score = free_bacon(opponent_score)
